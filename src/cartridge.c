...
const CartridgeType CartridgeTypes[] = {
		{0, 0, 0, 0, 0, 0}, // 00
		{1, 0, 0, 0, 0, 0}, // 01
		{1, 1, 0, 0, 0, 0}, // 02
		{1, 1, 1, 0, 0, 0}, // 03
...
void load_info(){
	cartridgeInfo.type = CartridgeTypes[cartridgeHeader.type];
	cartridgeInfo.GBC_MODE = cartridgeHeader.gbc_flag >> 7;
...
u8 load_header(char * fn) {
	...
	if (f) {
		...
		if (fread(&cartridgeHeader, sizeof(CartridgeHeader), 1, f) == 1) {
			load_info();
			...
			if (hchecksum() != cartridgeHeader.hchecksum)
				Log(Error, "Invalid header checksum", "cartridge: %02X / verified: %02X\n", cartridgeHeader.hchecksum,
				    hchecksum());
			else if ((cartridgeInfo.ram_size > 0) ^ cartridgeInfo.type.has_ram)
				Log(Error, "Incoherence between RAM specs", "has ram: %d / ram size: %u\n", cartridgeInfo.type.has_ram,
				    cartridgeInfo.ram_size);
			else {
				fclose(f);
				return 1;
			}
		} else Log(Critical, "Cannot read header", "\n");
		fclose(f);
	} else Log(Critical, "Cartridge not found", "file: '%s'\n", fn);
	return 0;
}
...
u8 load_cartridge(char * fn) {
	...
	if (f) {
		if (fread(Memory, 1, cartridgeInfo.rom_size, f) == cartridgeInfo.rom_size) {
			Log(Info, "Cartridge loaded", "\n");
			
			u16 c = gchecksum();
			if (c != cartridgeHeader.checksum) Log(Error, "Invalid global checksum", "header: %04X / computed: %04X\n", cartridgeHeader.checksum, c);

			fclose(f);
			return 1;
		} else Log(Critical, "Cannot (fully) read cartridge", "\n");
		fclose(f);
	} else Log(Critical, "Cartridge not found", "file: '%s'\n", fn);
	return 0;
}
